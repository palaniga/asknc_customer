...
import 'package:Asknc_user/view_models/new_parcel.vm.dart';
import 'package:Asknc_user/views/pages/cart/widgets/amount_tile.dart';
import 'package:Asknc_user/views/pages/parcel/widgets/form_step_controller.dart';
import 'package:Asknc_user/views/pages/parcel/widgets/package_delivery_discount_section.dart';
import 'package:Asknc_user/views/pages/parcel/widgets/parcel_order_payer.dart';
import 'package:Asknc_user/widgets/busy_indicator.dart';
import 'package:Asknc_user/widgets/cards/custom.visibility.dart';
import 'package:Asknc_user/widgets/cards/vendor_fees.view.dart';
import 'package:Asknc_user/widgets/custom_grid_view.dart';
import 'package:Asknc_user/widgets/list_items/payment_method.list_item.dart';
            ...
            CustomVisibilty(
              ...
              child: VStack(
                [
                  ...
                  AmountTile(
                    "Delivery Charges".tr(),
                    "${vm.currencySymbol} ${vm.packageCheckout.deliveryFee.toStringAsFixed(2)}"
                        .currencyFormat(),
                  ),
                  AmountTile(
                    "Package Size Charges".tr(),
                    "${vm.currencySymbol} ${vm.packageCheckout.packageTypeFee!.toStringAsFixed(2)}"
                        .currencyFormat(),
                  ),
                  DottedLine().py12(),
                  AmountTile(
                    "Subtotal".tr(),
                    "${vm.currencySymbol} ${vm.packageCheckout.subTotal!.toStringAsFixed(2) ?? ''}"
                        .currencyFormat(),
                  ),
                  AmountTile(
                    "Discount".tr(),
                    "- " +
                        "${vm.currencySymbol} ${vm.packageCheckout.discount!.toStringAsFixed(2)}"
                            .currencyFormat(),
                  ),
                  DottedLine().py12(),
                  AmountTile(
                    "Tax".tr() + " (${vm.packageCheckout.taxRate}%)",
                    "${vm.currencySymbol} ${vm.packageCheckout.tax!.toStringAsFixed(2) ?? ''}"
                        .currencyFormat(),
                  ),
                  //fees
                  VendorFeesView(
                    fees: vm.packageCheckout.vendor!.fees,
                    subTotal: vm.packageCheckout.subTotal!.toStringAsFixed(2).toDouble(),
                  ),
                  DottedLine().py12(),
                  //total
                  AmountTile(
                    "Total".tr(),
                    "${vm.currencySymbol} ${(vm.packageCheckout.total)!.toStringAsFixed(2)}"
                        .currencyFormat(),
                  ),
                ],
              ),
