...
import 'package:Asknc_user/views/pages/delivery_address/widgets/address_search.view.dart';
...
//step 1
  ...
  void updateGoogleMapPadding({required double height, Driver? driverInfo}) {
  ...
  // void driverInfo(Driver driverInfo){
  //
  //   notifyListeners();
  // }
  ...
  void setSourceAndDestinationIcons() async {
    ...
    if (driver?.vehicle != null) {
      icons = driver!.vehicle!.vehicleType == "BIKE"
          ? AppImages.bikeIcon
          : driver!.vehicle!.vehicleType == "AUTO"
              ? AppImages.auto
              : AppImages.driverCar;
    } else {}
...
//
  ...
  List<LatLng> _fullDriverRoute = []; // stores full original route
  Timer? _animationTimer;
  Future<void> drawDriverPickupPolyLines({
    required LatLng driverPosition,
    required LatLng pickupPosition,
  }) async {
    try {
      if (driverPosition.latitude == 0.0 && driverPosition.longitude == 0.0)
        return;
      if (pickupPosition.latitude == 0.0 && pickupPosition.longitude == 0.0)
        return;

      // --- Fetch full route only once ---
      if (_fullDriverRoute.isEmpty) {
        final polylineResult = await polylinePoints.getRouteBetweenCoordinates(
          ...
          PointLatLng(pickupPosition.latitude, pickupPosition.longitude),
        );

        _fullDriverRoute = polylineResult.points
            .map((p) => LatLng(p.latitude, p.longitude))
            .toList();
      }

      if (_fullDriverRoute.isEmpty) return;

      // --- Find Closest Point in Route ---
      LatLng? closestPoint;
      double minDistance = double.infinity;
      for (final point in _fullDriverRoute) {
      ...
      final closestIndex = _fullDriverRoute.indexOf(closestPoint);
      final visibleRoute = _fullDriverRoute.sublist(closestIndex);
      ...
      // --- If driver is near pickup, remove polyline ---
      ...
      gMapPolylines.add(
        Polyline(
          polylineId: const PolylineId("driverPickupPoly"),
          color: Colors.green,
          width: 4,
          points: visibleRoute,
        ),
      ...
      debugPrint("Error updating driver-pickup polyline: $e");
    }
  }
